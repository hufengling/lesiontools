...
#' @param mimosa_mask A binary mask identifying lesions above a threshold.
...
#' @importFrom ALPACANet lesion_centers split_confluent
...
#'
label_lesion <- function(prob_map, mimosa_mask, mincluster = 100) {
  labeled_image <- labelClusters(mimosa_mask, minClusterSize = 1, 
                                 fullyConnected = TRUE)
  ...
  lesion_center_image <- lesion_centers(
    prob_map = prob_map, mimosa_mask = mimosa_mask,
